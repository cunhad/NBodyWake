...
z=atof(argv[1]);
Gmu=1.*pow(10.,atof(argv[2]));
box_size=atof(argv[7]);
n_particles=atof(argv[8]);

cout << box_size << " " << n_particles << " " << z*Gmu << endl;
...
//        filename =argv[1]+argv[2]+argv[3];
//          filenameOut = argv[1]+argv[2]+argv[4];
...
float LinConv={*box_sizeP/(*n_particlesP)}; /*spatial linear convertion factor from simultion to comoving*/
...
//ifstream CatalCubep3mInit("/home/acer/Documents/storage/wake_comparizon/20.000xv0.dat", ios::binary);
//ofstream CatCubep3mOut("/home/acer/Documents/storage/wake_comparizon/20.000xv0_wake1.dat");
