function [ size_box, nc, np, zi, wake_or_no_wake ,multiplicity_of_files ,Gmu ,ziw ,z, path_file_in, Pos  ] = preprocessing_part_fseek( path,spec,aux_path,filename,part,part_id)
...
%(example) [ size_box, nc, np, zi, wake_or_no_wake ,multiplicity_of_files ,Gmu ,ziw ,z, path_file_in, Pos ] = preprocessing_part_fseek( '/home/asus/Dropbox/extras/storage/graham/small_res/','64Mpc_96c_48p_zi255_nowakem','/sample1001/','0.000xv0.dat',8,1);
...
%             fread(fid, [6 1], 'float32','l');                            
%         end
        fseek(fid,4*6*(part_chunck_start-particle_sum_per_file(node_ID)-1),'cof');
        header=true;
    end
    if (part_chunck_end<=particle_sum_per_file(node_ID+1))
        if header
        ...
        data_file=fread(fid, [6 part_chunck_end-particle_sum_per_file(node_ID)], 'float32','l');            
        end
    else 
