...
%   (example) [ cell_bins1d_z  count_sum Pos_halos mass_halos radius_halos count_sum_h_number count_sum_h_mass count_sum_dmdc] = proj1d_halos_data_out( '/home/asus/Dropbox/extras/storage/guillimin/old/', '/home/asus/Dropbox/extras/storage/guillimin/old/','32Mpc_96c_48p_zi63_nowakes','/','','31.000halo0.dat',1,1,[0,0,0],[0,0])
%   (example) [ cell_bins1d_z  count_sum Pos_halos mass_halos radius_halos count_sum_h_number count_sum_h_mass count_sum_dmdc] = proj1d_halos_data_out( '/home/asus/Dropbox/extras/storage/','/home/asus/Dropbox/extras/storage/','40Mpc_192c_96p_zi65_nowakes','/','','0.000halo0.dat',1,1,[0,0,0],[0,0])
...
count_sum_h_number=zeros(1,numel(cell_bins1d_z));
count_sum_h_mass=zeros(1,numel(cell_bins1d_z));
...
for node = 1 : length(nodes_list)
    ...
    if (~isempty(Pos_h))
        ...
        [count_h_n edges mid loc] = histcn(transpose(Pos_h),1,1,cell_bins1d_z);
        count_h_n=count_h_n(1:1,1:1,1:numel(cell_bins1d_z)-1);
        %     average=mean2(count);
        ...
        %    cell_bins1d(end)=[];
        count_sum_h_number=count_sum_h_number+transpose(count_h_n);
        
        
        [count_h_m edges mid loc] = histcn(transpose(Pos_h),1,1,cell_bins1d_z,'AccumData',transpose(mass));
        count_h_m=count_h_m(1:1,1:1,1:numel(cell_bins1d_z)-1);
        %     average=mean2(count);
        %     count=(count-average)/average;
        ...
        %    cell_bins1d(end)=[];
        count_sum_h_mass=count_sum_h_mass+transpose(count_h_m);  
...
dlmwrite(strcat(path_out,'aux/mass/','_',num2str(find(str2num(char(redshift_list))==z)),'_1dproj_mass_halos_z',num2str(z),'_data.txt'),mass_halos,'delimiter','\t');
mkdir(path_out,'aux/radius/');
dlmwrite(strcat(path_out,'aux/radius/','_',num2str(find(str2num(char(redshift_list))==z)),'_1dproj_radius_halos_z',num2str(z),'_data.txt'),radius_halos,'delimiter','\t');
