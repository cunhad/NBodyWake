function [ count_sum ] = proj1d_cic_dm_data_out( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,pivot,rot_angle,data_stream)
...
% cell_bins1d_z(end)=[];
count_sum=zeros(1,(np*resol_factor/lenght_factor));
...
%     [ ~,~,~,~,~,~,~,~,~,~,Pos ] = preprocessing_part(root,spec,aux_path,filename,length(nodes_list),node);
    ...
    lim_pre=(1/(2*lenght_factor))*np*resol_factor;
    ...
    count=zeros(1,(np*resol_factor/lenght_factor));
    ...
    for particle=1:length(dz)
       ...
       dx2=1-dx1;
        ...
        if (i1>0 && i2< (np*resol_factor/lenght_factor)) 
         count(i1)=count(i1)+dx1;
         count(i2)=count(i2)+dx2;
        end
        
    end
    
%     count=histcounts(dz,cell_bins1d_z);
