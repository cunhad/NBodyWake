function [ count_sum] = proj2d_cic_dm_data_out_part_rand_wake_slices( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,data_stream,particl_part,numb_angl,slice,NSIDE)
%Computes the 2d projections aconding to the input specifications and stores (and returns) the resulting data

%   (example) [ cell_bins1d_y,cell_bins1d_z,count_sum] = proj2d_cic_dm_data_out_part_rand_wake_slices('/home/asus/Dropbox/extras/storage/graham/small_res/','/home/asus/Dropbox/extras/storage/graham/small_res/data/','64Mpc_96c_48p_zi255_nowakem','/sample1001/','','0.000xv0.dat',1,1,[0,0,0],[0,0],[1,2],64);
%   (example) [ count_sum] = proj2d_cic_dm_data_out_part_rand_wake_slices('/home/asus/Dropbox/extras/storage/graham/small_res/','/home/asus/Dropbox/extras/storage/graham/small_res/data/','64Mpc_256c_128p_zi63_nowakem','/sample2001/','','10.000xv0.dat',1,1,1,1,1,4);
%   (example) [ cell_bins1d_y,cell_bins1d_z,count_sum] = proj2d_cic_dm_data_out_part_rand_wake_slices('/home/asus/Dropbox/extras/storage/guillimin/','/home/asus/Dropbox/extras/storage/guillimin/data/','64Mpc_1024c_512p_zi63_wakeGmu5t10m6zi63m','/sample0001/','','10.000xv0.dat',1,1,[0,0,0],[0,0],[1,2],64);
...
root_out='/home/disraelcunha/Dropbox/extras/storage/graham/small_res/data_test3_wake/';
...
prz=randi([-rand_amplitude_Z rand_amplitude_Z])

ra_along_wake=-pi/2+rand()*pi
ra_perp_wake=-pi/2+(numb_angl/Ntotal)*pi
...
pivot=[prx,0,prz]
rot_angle=[pi/2,ra_perp_wake,ra_along_wake]
...
if ~ismember(0,data_stream)
    ...
    for count_slice=1:slice
        ...
        if ismember(10,data_stream)
            ...
            set(gca,'YTick',[])
            set(gca,'XTick',[])
            saveas(fig,char(strcat(path_out_fig,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_col_2dproj_z',num2str(z_glob),'_data_sl',num2str(count_slice),'_tg16  _fig','.png')));
            close(fig);
        end
