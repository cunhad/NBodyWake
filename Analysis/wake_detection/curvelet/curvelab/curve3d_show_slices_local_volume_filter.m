...
% sample_id_range_nw=[3,6,7];
% sample_id_range_w=[3,6,7];
...
% display_slice_nw{find(sample_id_range_nw==3)}=[9];
% display_slice_nw{find(sample_id_range_nw==6)}=[30];
% display_slice_nw{find(sample_id_range_nw==7)}=[13];
% display_slice_w{find(sample_id_range_w==3)}=[29];
% display_slice_w{find(sample_id_range_w==6)}=[31];
% display_slice_w{find(sample_id_range_w==7)}=[28];

% display_slice_nw{find(sample_id_range_nw==3)}=[9*partition2d,29*partition2d];
% display_slice_nw{find(sample_id_range_nw==6)}=[30,31];
% display_slice_nw{find(sample_id_range_nw==7)}=[13*partition2d,28*partition2d];
% display_slice_w{find(sample_id_range_w==3)}=[9*partition2d,29*partition2d];
% display_slice_w{find(sample_id_range_w==6)}=[30,31];
% display_slice_w{find(sample_id_range_w==7)}=[13*partition2d,28*partition2d];
...
specs_path_list_nowake='/home/asus/Dropbox/extras/storage/graham/ht/data_cps128_1024/4Mpc_2048c_1024p_zi63_nowakem'
...
specs_path_list_wake='/home/asus/Dropbox/extras/storage/graham/ht/data_cps128_1024/4Mpc_2048c_1024p_zi63_wakeGmu1t10m7zi10m'
...
F2 = ones(nc_anal3d,nc_anal3d,slices);
...
F2=zeros(nc_anal3d,nc_anal3d,slices);
...
fig_test1_max3=figure;
fig_test2_max3=figure;
fig_test3_max3=figure;
fig_test4_max3=figure;
fig_test5_max3=figure;
...
ax_test1_max3=axes(fig_test1_max3);
ax_test2_max3=axes(fig_test2_max3);
ax_test3_max3=axes(fig_test3_max3);
ax_test4_max3=axes(fig_test4_max3);
ax_test5_max3=axes(fig_test5_max3);
...
%     for sample = 1:2
        ...
        map_3d_slices=zeros(nc_anal2d,nc_anal2d,slices);
        map_3d_slices_filt2d=zeros(nc_anal2d,nc_anal2d,slices);
...
%             if false
            if ismember(slice_id,display_slice{find(sample_id_range==sample)})
                ...
                figure; imagesc((size_mpc/nc_anal2d)*[1:nc_anal2d],(size_mpc/nc_anal2d)*[1:nc_anal2d],map_3d_slices(:,:,slice_id)); colorbar; axis('image');
            ...
            for s = length(C)-lev_2d:length(C)-1
                %                 thresh=0;
                thresh = sigma_2d + sigma_2d*(s == length(C));
                for w = 1:length(C{s})
                    %                     Ct{s}{w} = C{s}{w};
            ...
            if ismember(slice_id,display_slice{find(sample_id_range==sample)})
                ...
                figure; imagesc((size_mpc/nc_anal2d)*[1:nc_anal2d],(size_mpc/nc_anal2d)*[1:nc_anal2d],map_3d_slices_filt2d(:,:,slice_id)); colorbar; axis('image');
                xlabel('$Z(Mpc/h)$', 'interpreter', 'latex', 'fontsize', 20);
                ylabel('$Y(Mpc/h)$', 'interpreter', 'latex', 'fontsize', 20);
                set(gca,'FontName','FixedWidth');
                set(gca,'FontSize',16);
            ...
            frac_cut=0.5;
            R_nor=R;
            R_nor(R_u>nc_anal2d*frac_cut)=R_nor(R_u>nc_anal2d*frac_cut)./R_u(R_u>nc_anal2d*frac_cut);
            R_nor(R_u<=nc_anal2d*frac_cut)=0;
            
            boudary_removal_factor=2048/nc_anal2d;
            ...
            for i=1:length(R_nor(1,:))
                ...
                D(length(xp)-nc_anal2d*wavel_removal_factor:end)=0;
                D(1:nc_anal2d*wavel_removal_factor)=0;
                
                R_nor_filt(:,i)=D;
            end
            %             R_nor_filt(floor((2465-200)/boudary_removal_factor):end,:)=[];
            %             R_nor_filt(1:floor((448+200)/boudary_removal_factor),:)=[];
            R_nor_filt(length(xp)-nc_anal2d*wavel_removal_factor:end,:)=[];
            R_nor_filt(1:nc_anal2d*wavel_removal_factor,:)=[];
...
%         
        ...
        map_3d_slices_filt2a3d=zeros(nc_anal2d,nc_anal2d,slices);
...
%             C_aux=map_3d_slices_filt2d(:,:,(partition)*slices/2+1:(partition)*slices/2+slices/2);
            C_aux=map_3d_slices_filt2d((partition_x)*nc_anal2d/partition2d+1:(partition_x)*nc_anal2d/partition2d+nc_anal2d/partition2d,(partition_y)*nc_anal2d/partition2d+1:(partition_y)*nc_anal2d/partition2d+nc_anal2d/partition2d,1:slices);
%             C_aux_shifted=map_3d_slices_filt2d(mod((partition_x)*nc/2+1,nc:mod((partition_x)*nc/2+nc/2,nc),(partition_y)*nc/2+1):(partition_y)*nc/2+nc/2,:);
%           C_aux=map_3d_slices_filt2d(mod((partition_x)*nc/2+1,nc):mod((partition_x)*nc/2+nc/2,nc),mod((partition_y)*nc/2+1,nc):mod((partition_y)*nc/2+nc/2,nc),:);
%          if (partition_x==0)&(partition_y==0) 
%              C_aux_shifted=map_3d_slices_filt2d(mod((partition_x)*nc/2+1+nc/4,nc):mod((partition_x)*nc/2+nc/2+nc/4,nc),mod((partition_y)*nc/2+1+nc/4,nc):mod((partition_y)*nc/2+nc/2+nc/4,nc),1:slices);
%              C_shifted = fdct3d_forward(C_aux_shifted);
%          end
          ...
          C = fdct3d_forward(C_aux);
            ...
            for s =length(Ct)-lev_3d:length(Ct)-1
                %                 thresh=0;
                thresh = sigma_3d + sigma_3d*(s == length(C));
                for w = 1:length(C{s})
                    Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > 0*E2{s}{w})
                    %                                   Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > thresh*E{s}{w});
                    %                   Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > -thresh*E{s}{w}&(C{s}{w}) < thresh*E{s}{w});
...
%             map_3d_slices_filt2a3d(:,:,(partition)*slices/2+1:(partition)*slices/2+slices/2) = real(fdct3d_inverse(Ct));
            map_3d_slices_filt2a3d((partition_x)*nc_anal2d/partition2d+1:(partition_x)*nc_anal2d/partition2d+nc_anal2d/partition2d,(partition_y)*nc_anal2d/partition2d+1:(partition_y)*nc_anal2d/partition2d+nc_anal2d/partition2d,1:slices) = real(fdct3d_inverse(Ct));            
...
%             if false            
            if ismember(slice_id,display_slice{find(sample_id_range==sample)})
                ...
                figure; imagesc((size_mpc/nc_anal2d)*[1:nc_anal2d],(size_mpc/nc_anal2d)*[1:nc_anal2d],map_3d_slices_filt2a3d(:,:,slice_id)); colorbar; axis('image');
                xlabel('$Z(Mpc/h)$', 'interpreter', 'latex', 'fontsize', 20);
                ylabel('$Y(Mpc/h)$', 'interpreter', 'latex', 'fontsize', 20);
                set(gca,'FontName','FixedWidth');
                set(gca,'FontSize',16);
...
%             test4max2_lv{sample_id}=   plot(ax_test4_max2,d_max2,coul);
         ...
         for slice_id=1:slices/sum_depth
            
            
            this=sum(map_3d_slices_filt2a3d(:,:,(slice_id-1)*sum_depth+1:slice_id*sum_depth),3);
            map_3d_slices_filt2a3d_depthsum(:,:,slice_id)=this;
            
%             if false            
            if ismember((slice_id-1)*sum_depth+1,display_slice{find(sample_id_range==sample)})
                ...
                set(gca,'FontSize',16);
                set(gca,'linewidth',2);
                title(strcat('filt2a3 for sample ',num2str(sample),' slice ',num2str((slice_id-1)*sum_depth+1)));
            end
            
            theta = 0:step_of_degree:180;
            [R,xp] = radon(this,theta);
...
%                 figure; imagesc((size_mpc/nc)*[1:nc],(size_mpc/1024)*[1:nc],map_3d_slices_filt2a3d(:,:,slice_id)); colorbar; axis('image');
                ...
                set(gca,'FontSize',16);
                set(gca,'linewidth',2);
                title(strcat('wfilt ridg filt2a3 for sample ',num2str(sample),' slice ',num2str((slice_id-1)*sum_depth+1)));
            ...
            anali_sum3(w_nw,sample,slice_id,1,:)=[max(this(:)),std(this(:)),max(this(:))/std(this(:)),kurtosis(kurtosis(this)),kurtosis(this(:))];
            anali_sum3(w_nw,sample,slice_id,2,:)=[max(R(:)),std(R(:)),max(R(:))/std(R(:)),kurtosis(kurtosis(R)),kurtosis(R(:))];
            anali_sum3(w_nw,sample,slice_id,3,:)=[max(R_nor(:)),std(R_nor(:)),max(R_nor(:))/std(R_nor(:)),kurtosis(kurtosis(R_nor)),kurtosis(R_nor(:))];
%             anali_sum3(w_nw,sample,slice_id,4,:)=[max(R_nor_filt2(:)),std(R_nor_filt2(:)),max(R_nor_filt2(:))/std(R_nor_filt2(:)),kurtosis(kurtosis(R_nor_filt2)),kurtosis(R_nor_filt2(:))];
            anali_sum3(w_nw,sample,slice_id,4,:)=[max(R_nor_filt(:)),std(R_nor_filt(:)),max(R_nor_filt(:))/std(R_nor_filt(:)),kurtosis(kurtosis(R_nor_filt)),kurtosis(R_nor_filt(:))];
            anali_sum3(w_nw,sample,slice_id,5,:)=[max(R_2dwav_filt(:)),std(R_2dwav_filt(:)),max(R_2dwav_filt(:))/std(R_2dwav_filt(:)),kurtosis(kurtosis(R_2dwav_filt)),kurtosis(R_2dwav_filt(:))];
%             a_max2(:)=anali_sum2(w_nw,sample,slice_id,1,:);
%             b_max2(:)=anali_sum2(w_nw,sample,slice_id,2,:);
%             c_max2(:)=anali_sum2(w_nw,sample,slice_id,3,:);
%             d_max2(:)=anali_sum2(w_nw,sample,slice_id,4,:);
...
%             test4max2_lv{sample_id}=   plot(ax_test4_max2,d_max2,coul);
            ...
            a_max3(:)=max(anali_sum3(w_nw,sample,:,1,:),[],3);
            b_max3(:)=max(anali_sum3(w_nw,sample,:,2,:),[],3);
            c_max3(:)=max(anali_sum3(w_nw,sample,:,3,:),[],3);
            d_max3(:)=max(anali_sum3(w_nw,sample,:,4,:),[],3);
            e_max3(:)=max(anali_sum3(w_nw,sample,:,5,:),[],3);
            
            test1max3_lv{sample_id}=   plot(ax_test1_max3,a_max3,coul);
            test2max3_lv{sample_id}=   plot(ax_test2_max3,b_max3,coul);
            test3max3_lv{sample_id}=   plot(ax_test3_max3,c_max3,coul);
            test4max3_lv{sample_id}=   plot(ax_test4_max3,d_max3,coul);
            test5max3_lv{sample_id}=   plot(ax_test5_max3,e_max3,coul);
        
        clearvars a b c d aa a_max b_max c_max d_max a_max2 b_max2 c_max2 d_max2 e_max2 a_max3 b_max3 c_max3 d_max3 e_max3
...
%         
        ...
        hold(ax_test1_max3,'on');
        hold(ax_test2_max3,'on');
        hold(ax_test3_max3,'on');
        hold(ax_test4_max3,'on');
        hold(ax_test5_max3,'on');        
%         
    end
...
set(ax_test1_max3, 'YScale', 'log');
title(ax_test1_max3,'map curvelet from 2dcurv plus sum curv');
set(ax_test2_max3, 'YScale', 'log');
title(ax_test2_max3,'ridgelet from sum 2dcurv sum curv');
set(ax_test3_max3, 'YScale', 'log');
title(ax_test3_max3,'ridgelet normalized from 2dcurv plus sum curv');
set(ax_test4_max3, 'YScale', 'log');
title(ax_test4_max3,'ridgelet normalized with wavelet from 2dcurv plus sum curv');
set(ax_test5_max3, 'YScale', 'log');
title(ax_test5_max3,'ridgelet normalized with 2dwavelet from 2dcurv plus sum curv');
