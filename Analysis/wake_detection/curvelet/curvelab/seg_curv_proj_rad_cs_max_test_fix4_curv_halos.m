function [ anali,anali_sum,anali_sum2 ] = seg_curv_proj_rad_cs_max_test_fix4_curv_halos(  )
...
filename='_2dproj_halos_n_z3_data_sl';
nc=1024;
trsh=20;
cut=1;
lev=2;
...
specs_path_list_nowake='/home/asus/Dropbox/extras/storage/graham/ht/data_halo_32sli_1024/4Mpc_2048c_1024p_zi63_nowakem'
...
specs_path_list_wake='/home/asus/Dropbox/extras/storage/graham/ht/data_halo_32sli_1024/4Mpc_2048c_1024p_zi63_wakeGmu1t10m7zi10m'
...
% path_out_root='/home/asus/Dropbox/extras/storage/graham/ht/data_halos32_1024_2a3dcurvfilt/';
% 
% % mkdir(path_out_root);
...
%     for sample = 1:2
        ...
        for slice_id=1:slices
            ...
            filename_nowake=strcat('',specs_path_list,'/',string(sample_list(sample)),'/data/1lf_1rf_0-0-0pv_1.5708-0ra/2dproj/halos/number/',ch,filename,num2str(slice_id),'.bin')
...
%             
%             fileID = fopen(strcat(path_out,ch,filename,num2str(slice_id),'_curvfilt2a3d','.bin'),'w');
%             fwrite(fileID,this, 'float32','l');
%             fclose(fileID);
%             
% %             dlmwrite(strcat(path_out,ch,filename,num2str(slice_id),'_curvfilt2d','.txt'),this,'delimiter','\t');
...
%             
%             
%             mkdir(path_out_root,char(strcat(path_out_waketype,string(sample_list(sample)),'/data/1lf_1rf_0-0-0pv_1.5708-0-0ra/2dproj/dm/2a3d_curvfilt/')));
%             path_out=strcat(path_out_root,path_out_waketype,string(sample_list(sample)),'/data/1lf_1rf_0-0-0pv_1.5708-0-0ra/2dproj/dm/2a3d_curvfilt/');            
%             
%             
% %             firstfig=this;
% %             secondfig=map_3d_slices_filt2a3d(:,:,mod(slice_id+1,slices));
% %             third=map_3d_slices_filt2a3d(:,:,mod(slice_id+1,slices));
%             
% %             dlmwrite(strcat(path_out,ch,filename,num2str(slice_id),'_curvfilt2d','.txt'),this,'delimiter','\t');
%              
...
% %               fig=figure;
%             
%             set(gcf, 'Position', [0 0 1023 1023]);
%             hold on;
%             axes('Position',[0 0 1 1],'Visible','off');
%             
%             imagesc([0 1023],[0 1023],repmat(this,[1 1 3]));
%             f=getframe(gcf);
%             new=f.cdata;
% %             imwrite(new,strcat(path_out,ch,filename,num2str(slice_id),'_fcurvfilt2a3d','.png'),'PNG');
%             saveas(fig,char(strcat(path_out,ch,filename,num2str(slice_id),'_fcurvfilt2a3d','.png')));
%             close(fig);
%             
%             
%             fig=figure('Visible', 'off');
...
%             
% %             colorfigure(:,:,1)=this;
% %             colorfigure(:,:,2)=map_3d_slices_filt2a3d(:,:,mod(slice_id+1,slices-1)+1);
% %             colorfigure(:,:,3)=map_3d_slices_filt2a3d(:,:,mod(slice_id+2,slices-1)+1);
% %             
%             
%             colorfigure(:,:,1)=map_3d_slices_filt2a3d(:,:,mod(slice_id-1-1,slices)+1);
%             colorfigure(:,:,2)=this;
%             colorfigure(:,:,3)=map_3d_slices_filt2a3d(:,:,mod(slice_id+1-1,slices)+1);
% 
%             set(gcf, 'Position', [0 0 1023 1023]);
%             hold on;
%             axes('Position',[0 0 1 1],'Visible','off');
%             imagesc([0 1023],[0 1023],colorfigure);
%             f=getframe(gcf);
%             new=f.cdata;
% %             imwrite(new,strcat(path_out,ch,filename,num2str(slice_id),'_fcurvfilt2a3d3col','.png'),'PNG');
%             saveas(fig,char(strcat(path_out,ch,filename,num2str(slice_id),'_fcurvfilt2a3d3col','.png')));
%             close(fig);
%             
% %             close(fig)
% % 
...
% % [ anali,anali_sum,anali_sum2 ] = seg_curv_proj_rad_cs_max_test_fix4_curv_figout(  );
% nowake=reshape(permute(anali_sum2(1,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))])
% wake=reshape(permute(anali_sum2(2,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))])
% mean_wake=mean(wake)
...
% mean_stn-std_stn
% wake_slices = reshape(wake,[10,32])'
% nowake_slices = reshape(nowake,[10,32])'
...
% mean_stn-std_stn
 ...
 %[ anali,anali_sum,anali_sum2 ] = seg_curv_proj_rad_cs_max_test_fix4_curv(  );
nowake=reshape(permute(anali_sum2(1,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))])
wake=reshape(permute(anali_sum2(2,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))])
mean_wake=mean(wake)
mean_nowake=mean(nowake)
...
% dlmwrite(strcat(path_out_root(1:end-1),'_all','/curvfilt2a3d_label_numbers_statistics','.txt'),label_numbers_statistics,'delimiter','\t');
% dlmwrite(strcat(path_out_root(1:end-1),'_all','/curvfilt2a3d_label','.txt'),label_number,'delimiter','\t');


%ks test

sig_nw=reshape(permute(anali_sum2(1,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))]);
sig_w=reshape(permute(anali_sum2(2,:,:,4,1),[1,3,2,4,5]),[1,numel(anali(1,:,:,2,1))]);
signal_nw=sig_nw(:);
signal_w=sig_w(:);
