function [ anali,anali_curv_mom,anali3,anali3_curv_mom,sample_id_range_nw,sample_id_range_w ] = curve3d_show_slices_minimal_nor_locangreal_Cnsq_d1t5(  )


%here I tries to fiz the previous limit,of it do not doing the
%normalization, just uses the bsolute value of the curvelet parameter. Now
%we introduce the normalization, and also the angular normalization (that is, angular weight)

%good potential.2 (try stuff, tipo o campo de spin)
...
% sample_id_range_nw=[4,7]; sample_id_range_w=[3,7];
% sample_id_range_nw=[4,7]; sample_id_range_w=[4,7];
% 
% sample_id_range_nw=[1:10]; sample_id_range_w=[1:10];

% sample_id_range_nw=[4,6,7]; sample_id_range_w=[4,6,7];
...
% display_slice_nw={[],[]}; display_slice_w={[],[]};

% sample_id_range=[3, 7]; sample_id_range=[1 : length(sample_list_nowake)];
...
%
% F=zeros(nc); C_zero = fdct_wrapping(F,0);
F = ones(nc);
...
% 
% fig_curv=figure; fig_test1=figure;
fig_test2=figure;
fig_test3=figure;
fig_test4=figure;

% fig_test1_m=figure; fig_test2_m=figure; fig_test3_m=figure;
% fig_test4_m=figure;

% fig_test1_max=figure; fig_test2_max=figure; fig_test3_max=figure;
% fig_test4_max=figure;

% fig_test1_max2=figure; fig_test2_max2=figure; fig_test3_max2=figure;
% fig_test4_max2=figure; fig_test5_max2=figure;

% fig_test1_curv=figure; fig_test2_curv=figure; fig_test3_curv=figure;
...
% ax_curv=axes(fig_curv); ax_test1=axes(fig_test1);
ax_test2=axes(fig_test2);
ax_test3=axes(fig_test3);
ax_test4=axes(fig_test4);

% ax_test1_m=axes(fig_test1_m); ax_test2_m=axes(fig_test2_m);
% ax_test3_m=axes(fig_test3_m); ax_test4_m=axes(fig_test4_m);
% 
% ax_test1_max=axes(fig_test1_max); ax_test2_max=axes(fig_test2_max);
% ax_test3_max=axes(fig_test3_max); ax_test4_max=axes(fig_test4_max);

% ax_test1_max2=axes(fig_test1_max2); ax_test2_max2=axes(fig_test2_max2);
% ax_test3_max2=axes(fig_test3_max2); ax_test4_max2=axes(fig_test4_max2);
% ax_test5_max2=axes(fig_test5_max2);

% ax_test1_curv=axes(fig_test1_curv); ax_test2_curv=axes(fig_test2_curv);
...
%     for sample = 1:length(sample_id_range) for sample = 1:2
...
%             map = imresize(map,new_nc/nc,'triangle');
            ...
            %         dc=map; dc(dc>cut)=cut;
            
            %         nc_red=nc/red; conv_=ones(red); dc_red =
            %         conv2(dc,conv_,'valid'); dc =
            %         dc_red(1:red:end,1:red:end)/(red*red);
            %
            
            %
            %
            %             dc_cut=dc; dc_cut(dc_cut>cut)=cut; %
            %             dc_cut(dc_cut>cut)=-1; %         dc_cut =
            %             edge(dc_cut,'canny'); %
            %             dc=double(dc_cut);
            %
            %             thresh = multithresh(dc_cut,trsh); seg_I =
            %             imquantize(dc_cut,thresh); %         test=dc_cut;
            %             %         test=seg_I; test=log(seg_I);
%             map(map<=1)=1;%to remove problem with holes
%             map_3d_slices(:,:,slice_id)=log(map);
            %
            %             map(map<=1)=1;%to remove problem with holes
            %             map=map+1; test=log(log(map));
%             
            map(map<=1)=1;%to remove problem with holes
            map_3d_slices(:,:,slice_id)=log(map);            
...
%             C = fdct_wrapping(log(map),0); Ct = C; for s = 1:length(C)
%                 for w = 1:length(C{s})
%                     Ct{s}{w} = C_zero{s}{w};
%                 end
%             end
%             
%             aux_count=1; for s = length(C)-lev:length(C)-1
%                 %                 thresh=0; thresh = sigma + sigma*(s ==
%                 length(C)); for w = 1:length(C{s})
%                     %                     Ct{s}{w} = C{s}{w}; %
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > thresh*E{s}{w}); %
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > 0*E{s}{w});
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) >
%                     -thresh*E{s}{w}&(C{s}{w}) < thresh*E{s}{w}); %
%                     Ct{s}{w} = C{s}{w};
% %
                ...
                for sq_x=0:n_sq_d-1
                    for sq_y=0:n_sq_d-1
                        ...
                        for w = [1:1*length(C{s})/4,2*length(C{s})/4+1:3*length(C{s})/4]
                            ...
                            Ct{s}{w}(x_range_pts_ini:x_range_pts_fin,y_range_pts_ini:y_range_pts_fin)=max(normalization(1:x_range_pts_fin-x_range_pts_ini+1,1:y_range_pts_fin-y_range_pts_ini+1,w),Ct{s}{w}(x_range_pts_ini:x_range_pts_fin,y_range_pts_ini:y_range_pts_fin));
...
%                             filt_curv_sq(1:x_range_pts_fin-x_range_pts_ini+1,1:y_range_pts_fin-y_range_pts_ini+1,w)=abs(C{s}{w}(x_range_pts_ini:x_range_pts_fin,y_range_pts_ini:y_range_pts_fin));
                        ...
                        for w = [length(C{s})/4+1:2*length(C{s})/4,3*length(C{s})/4+1:4*length(C{s})/4]
                            ...
                            Ct{s}{w}(x_range_pts_ini:x_range_pts_fin,y_range_pts_ini:y_range_pts_fin)=max(normalization(1:y_range_pts_fin-y_range_pts_ini+1,1:x_range_pts_fin-x_range_pts_ini+1,w)',Ct{s}{w}(x_range_pts_ini:x_range_pts_fin,y_range_pts_ini:y_range_pts_fin));
...
%                 
%                 
% %                 sz=cell(length(C{s}),1); %                 for w =
% 1:length(C{s})/4 %                     sz{w}=size(C{s}{w}); %
% end
% 
%                 %right part
% 
...
%                             normalization=(a-avr_a)/avr_a; for w =
%                             1:length(C{s})/4
%                                 i_c=ceil(i*sz{w}(1)/sz{1}(1));
%                                 j_c=ceil(j*sz{w}(2)/sz{1}(2)); %
...
%                             normalization=(a-mean(a)/std_a);
%                             normalization=(a-avr_a)/avr_a; for w =
%                             1+2*length(C{s})/4:3*length(C{s})/4
%                                 i_c=ceil(i*sz{w}(1)/sz{1+2*length(C{s})/4}(1));
%                                 j_c=ceil(j*sz{w}(2)/sz{1+2*length(C{s})/4}(2));
%                                 Ct{s}{w}(i_c,j_c)=max(normalization(w),Ct{s}{w}(i_c,j_c));
%                             end
...
%                             normalization=(a-mean(a)/std_a);
%                             normalization=(a-avr_a)/avr_a; for w =
%                             1+3*length(C{s})/4:4*length(C{s})/4
%                                 i_c=ceil(i*sz{w}(1)/sz{1+3*length(C{s})/4}(1));
%                                 j_c=ceil(j*sz{w}(2)/sz{1+3*length(C{s})/4}(2));
%                                 Ct{s}{w}(i_c,j_c)=max(normalization(w),Ct{s}{w}(i_c,j_c));
%                             end
...
% 
%             if
%             ismember(slice_id,display_slice{find(sample_id_range==sample)})
%                 
%                 figure;
%                 imagesc(0:step_of_degree:180,(size_mpc/new_nc)*[1:new_nc],R);colorbar;
%                 xlabel('$\theta (degrees)$', 'interpreter', 'latex',
%                 'fontsize', 20); ylabel('$Z(Mpc/h)$', 'interpreter',
%                 'latex', 'fontsize', 20);
%                 set(gca,'FontName','FixedWidth'); set(gca,'FontSize',16);
%                 set(gca,'linewidth',2); title(strcat('wfilt ridg filt2a3
%                 for sample ',num2str(sample),' slice
%                 ',num2str(slice_id)));
%                 
%                 figure;
%                 imagesc(0:step_of_degree:180,(size_mpc/new_nc)*[1:new_nc],R_nor);colorbar;
%                 xlabel('$\theta (degrees)$', 'interpreter', 'latex',
%                 'fontsize', 20); ylabel('$Z(Mpc/h)$', 'interpreter',
%                 'latex', 'fontsize', 20);
%                 set(gca,'FontName','FixedWidth'); set(gca,'FontSize',16);
%                 set(gca,'linewidth',2); title(strcat('wfilt norm filt2
%                 for sample ',num2str(sample),' slice
%                 ',num2str(slice_id)));
%                 
%                 
%                 
%                 %                 figure;
%                 imagesc((size_mpc/nc)*[1:nc],(size_mpc/1024)*[1:nc],map_3d_slices_filt2a3d(:,:,slice_id));
%                 colorbar; axis('image'); figure;
%                 imagesc(0:step_of_degree:180,(size_mpc/new_nc)*[1:new_nc],R_nor_filt);colorbar;
%                 xlabel('$\theta (degrees)$', 'interpreter', 'latex',
%                 'fontsize', 20); ylabel('$Z(Mpc/h)$', 'interpreter',
%                 'latex', 'fontsize', 20);
%                 set(gca,'FontName','FixedWidth'); set(gca,'FontSize',16);
%                 set(gca,'linewidth',2); title(strcat('wfilt ridg filt2
%                 for sample ',num2str(sample),' slice
%                 ',num2str(slice_id))); % %                 figure;
%                 imagesc(0:step_of_degree:180,(size_mpc/new_nc)*[1:new_nc],R_2dwav_filt);colorbar;
%                 %                 xlabel('$\theta (degrees)$',
%                 'interpreter', 'latex', 'fontsize', 20); %
%                 ylabel('$Z(Mpc/h)$', 'interpreter', 'latex', 'fontsize',
%                 20); %                 set(gca,'FontName','FixedWidth');
%                 %                 set(gca,'FontSize',16); %
%                 set(gca,'linewidth',2); %
%                 title(strcat('wfilt ridg filt2a3 for sample
%                 ',num2str(sample),' slice ',num2str(slice_id)));
%             end
...
% hold(ax_curv,'on'); %             hold(ax_test1,'on');
%             hold(ax_test2,'on'); hold(ax_test3,'on');
%             hold(ax_test4,'on');
% 
%             hold(ax_test1_curv,'on'); hold(ax_test2_curv,'on');
...
%             
%             C = fdct3d_forward(C_aux); Ct = C; for s = 1:length(C)
%                 for w = 1:length(C{s})
%                     Ct{s}{w} = C_zero2{s}{w};
%                 end
%             end
%             
%             aux_count=1; for s = 1:length(C)
%                 %                 thresh=0; thresh = Sigma + Sigma*(s ==
%                 length(C)); for w = 1:length(C{s})
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) >  0*E2{s}{w}); %
%                     Ct{s}{w} = C{s}{w}; %                     Ct{s}{w} =
%                     C{s}{w}.* ((C{s}{w}) > thresh*E{s}{w}); %
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) >
%                     -thresh*E{s}{w}&(C{s}{w}) < thresh*E{s}{w}); %
%                     Ct{s}{w} = C{s}{w}; %
%                     curv(w_nw,sample,slice_id,w,aux_count)=kurtosis(abs(C{s}{w}(:)));
...
%             end
%             
%             map_3d_slices_filt3d(:,:,(partition)*slices/2+1:(partition)*slices/2+slices/2)
%             = real(fdct3d_inverse(Ct));
%             
%         end
%         
%         for slice_id=1:slices
...
%             
%             theta = 0:180; [R,xp] =
%             radon(map_3d_slices_filt3d(:,:,slice_id),theta);
%             
%             unit=ones(nc); [R_u,xp] = radon(unit,theta);
%             
%             frac_cut=0.5; R_nor=R;
...
%             
%             n_levels=floor(log2(length(R_nor(:,1))));
%             R_nor_filt=zeros(size(R_nor)); for i=1:length(R_nor(1,:))
%                 %                 [dc_dwt,levels] =
%                 wavedec(R_nor(:,i),n_levels,'db1'); [dc_dwt,levels] =
%                 wavedec(R(:,i),n_levels,'db1'); D =
%                 wrcoef('d',dc_dwt,levels,'db1',lev); %
%                 D(floor((2465-200)/boudary_removal_factor):end)=0; %
%                 D(1:floor((448+200)/boudary_removal_factor))=0;
%                 D(1237-256:end)=0; D(1:217+256)=0;
%                 
%                 R_nor_filt(:,i)=D;
%             end %
%             R_nor_filt(floor((2465-200)/boudary_removal_factor):end,:)=[];
%             %
%             R_nor_filt(1:floor((448+200)/boudary_removal_factor),:)=[];
%             R_nor_filt(1237-256:end,:)=[]; R_nor_filt(1:217+256,:)=[];
%             
...
%             C_aux=map_3d_slices_filt2d(:,:,:);
            ...
            for s = length(C)-lev_3d:length(C)-1
                ...
                for w = 1:length(C{s})
%                                         Ct{s}{w} = C_{s}{w}; Ct{s}{w} =
%                                         C_{s}{w}.* ((C_{s}{w}) >
%                                         thresh*E2{s}{w});
                                       Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > 0*C{s}{w});
%                     Ct{s}{w} = C_{s}{w}.* ((C_{s}{w}) >
%                     -thresh*E2{s}{w}&(C_{s}{w}) < thresh*E{2s}{w});
%                                     Ct{s}{w} = C{s}{w};
%                     curv(w_nw,sample,slice_id,w,aux_count)=kurtosis(abs(C{s}{w}(:)));
%                     collect=[ collect ;abs(Ct{s}{w}(:))]; collect_=[
%                     collect_ ;abs(Ct{s}{w}(:))/E{s}{w}];
...
%             R_nor_filt2=zeros(size(R_nor_filt)); for
%             i=1:length(R_nor_filt(:,1))
%                 %                 [dc_dwt,levels] =
%                 wavedec(R_nor(:,i),n_levels,'db1'); [dc_dwt,levels] =
%                 wavedec(R_nor_filt(i,:),n_levels2,'db1'); D =
%                 wrcoef('d',dc_dwt,levels,'db1',lev); %
%                 D(floor((2465-200)/boudary_removal_factor):end)=0; %
%                 D(1:floor((448+200)/boudary_removal_factor))=0;
% %                 D(length(xp)-new_nc*wavel_removal_factor:end)=0; %
...
% [H1,V1,D1] = detcoef2('all',c_2dwav,s_2dwav,1); % %             A1 =
% appcoef2(c_2dwav,s_2dwav,'haar',1); % %             V1img =
% wcodemat(V1,255,'mat',1); % %             H1img =
% wcodemat(H1,255,'mat',1); % %             D1img =
% wcodemat(D1,255,'mat',1); % %             A1img =
% wcodemat(A1,255,'mat',1); % %             A1_2dwav =
% appcoef2(c_2dwav,s_2dwav,'haar',1); %
% R_2dwav_filt=R_nor_filt-A1_2dwav;
% 
%             [cA1,cH1,cV1,cD1]=dwt2(orig_img,'haar'); %cA1(:)=0;
%             [cA2,cH2,cV2,cD2]=dwt2(cA1,'haar'); cA2(:)=0;
...
%         hold(ax_test1_m,'on'); hold(ax_test2_m,'on');
%         hold(ax_test3_m,'on'); hold(ax_test4_m,'on');
%         
%         hold(ax_test1_max,'on'); hold(ax_test2_max,'on');
%         hold(ax_test3_max,'on'); hold(ax_test4_max,'on');
%         
        hold(ax3_test1,'on');
        hold(ax3_test2,'on');
        hold(ax3_test3,'on');
...
%         
%         hold(ax_test1_max2,'on'); hold(ax_test2_max2,'on');
...
% 
% set(ax_curv, 'YScale', 'log'); title(ax_curv,'curvelet kurtosis')
% set(ax_test1, 'YScale', 'log'); title(ax_test1,'all map curvelet');
...
% 
% set(ax_test1_m, 'YScale', 'log'); title(ax_test1_m,'map curvelet sum
% max'); set(ax_test2_m, 'YScale', 'log'); title(ax_test2_m,'ridgelet from
% max'); set(ax_test3_m, 'YScale', 'log'); title(ax_test3_m,'ridgelet
% normalized from max'); set(ax_test4_m, 'YScale', 'log');
% title(ax_test4_m,'ridgelet normalized with wavelet from max');
% 
% 
% set(ax_test1_max, 'YScale', 'log'); title(ax_test1_max,'map curvelet from
% curv'); set(ax_test2_max, 'YScale', 'log'); title(ax_test2_max,'ridgelet
% from curv'); set(ax_test3_max, 'YScale', 'log');
% title(ax_test3_max,'ridgelet normalized from curv'); set(ax_test4_max,
% 'YScale', 'log'); title(ax_test4_max,'ridgelet normalized with wavelet
% from curv');
% 
% set(ax_test1_max2, 'YScale', 'log'); title(ax_test1_max2,'map curvelet
% from 2dcurv plus curv'); set(ax_test2_max2, 'YScale', 'log');
% title(ax_test2_max2,'ridgelet from sum 2dcurv curv'); set(ax_test3_max2,
% 'YScale', 'log'); title(ax_test3_max2,'ridgelet normalized from 2dcurv
% plus curv'); set(ax_test4_max2, 'YScale', 'log');
% title(ax_test4_max2,'ridgelet normalized with wavelet from 2dcurv plus
% curv'); set(ax_test5_max2, 'YScale', 'log');
% title(ax_test5_max2,'ridgelet normalized with 2dwavelet from 2dcurv plus
% curv');

% set(ax_test1_curv, 'YScale', 'log'); title(ax_test1_curv,'kurtosis
% normalised'); set(ax_test2_curv, 'YScale', 'log');
% title(ax_test2_curv,'kurtosis curvelet'); set(ax_test3_curv, 'YScale',
% 'log'); title(ax_test3_curv,'4th moment curvelet');
