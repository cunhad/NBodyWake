function [ anali ] = seg_curv_proj_rad_cs(  )
...
specs_path_list_nowake='/home/asus/Dropbox/extras/storage/graham/ht/data_cps8_2024/4Mpc_2048c_1024p_zi63_nowakem'
...
specs_path_list_wake='/home/asus/Dropbox/extras/storage/graham/ht/data_cps8_2024/4Mpc_2048c_1024p_zi63_wakeGmu1t10m7zi10m'
...
for w_nw=1:2
    ...
    for sample = 1:length(sample_id_range)
        ...
        for slice_id=1:slice
            ...
            filename_nowake=strcat('',specs_path_list,'/',string(sample_list(sample)),'/data/1lf_2rf_0-0-0pv_1.5708-0-0ra/2dproj/dm/',ch,filename,num2str(slice_id),'.bin')
            ...
            for s = length(C)-lev:length(C)
                ...
                for w = 1:length(C{s})
                    Ct{s}{w} = C{s}{w};
                    curv(w_nw,sample_id,w,aux_count)=kurtosis(abs(C{s}{w}(:)));
                end
                curv2(w_nw,sample_id,aux_count)=kurtosis(curv(w_nw,sample_id,:,aux_count))
            ...
            anali(w_nw,sample_id,1,:)=[max(BW2(:)),std(BW2(:)),max(BW2(:))/std(BW2(:)),kurtosis(kurtosis(BW2)),kurtosis(BW2(:))];
            ...
            anali(w_nw,sample_id,2,:)=[max(R(:)),std(R(:)),max(R(:))/std(R(:)),kurtosis(kurtosis(R)),kurtosis(R(:))];
            anali(w_nw,sample_id,3,:)=[max(R_nor(:)),std(R_nor(:)),max(R_nor(:))/std(R_nor(:)),kurtosis(kurtosis(R_nor)),kurtosis(R_nor(:))];
            anali(w_nw,sample_id,4,:)=[max(R_nor_filt(:)),std(R_nor_filt(:)),max(R_nor_filt(:))/std(R_nor_filt(:)),kurtosis(kurtosis(R_nor_filt)),kurtosis(R_nor_filt(:))];
            
            
            aa(:)=curv2(w_nw,sample_id,:);
            
            a(:)=anali(w_nw,sample_id,1,:);
            b(:)=anali(w_nw,sample_id,2,:);
            c(:)=anali(w_nw,sample_id,3,:);
            d(:)=anali(w_nw,sample_id,4,:);
