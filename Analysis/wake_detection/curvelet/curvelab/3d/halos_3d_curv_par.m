function [ cell_bins1d_x,cell_bins1d_y, cell_bins1d_z , Pos_halos ,mass_halos ,radius_halos, count_sum_h_number,list] = halos_3d_curv_par( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,pivot,rot_angle,num_cores)
%Computes the 2d projections aconding to the input specifications and stores (and returns) the resulting data


%   (example) halos_3d_curv_par( '/home/asus/Dropbox/extras/storage/graham/ht/', '/home/asus/Dropbox/extras/storage/graham/ht/data/','4Mpc_2048c_1024p_zi63_wakeGmu1t10m7zi10m','/sample3001/half_lin_cutoff_half_tot_pert_nvpw/','','3.000halo0.dat',1,1/4,[0,0,0],[pi/2,0,0],4);
...
path_out_plot=strcat(strcat(root_out,'plot/',spec,aux_path),aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv_',strcat(num2str(rot_angle(1)),'-',num2str(rot_angle(2)),'-',num2str(rot_angle(3))),'ra','/','3d/halos/');
mkdir(strcat(root_out,'plot/',spec,aux_path),strcat(aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv_',strcat(num2str(rot_angle(1)),'-',num2str(rot_angle(2)),'-',num2str(rot_angle(3))),'ra','/','3d/halos/'));
