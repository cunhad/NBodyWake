...
% root_data_2d_out='';
% root_anali_2d_out='/home/asus/Dropbox/extras/storage/graham/ht/data_cps32_1024_2dcla_s5lv2_anali/';
% root_visual_2d='/home/asus/Dropbox/extras/storage/graham/ht/data_cps32_1024_2dcla_s5lv2_visual/';
...
for slice_id=1:slices
    ...
    for  i=1:sz{1}(1)
        for j=1:sz{1}(2)
            ...
            for w = 1:length(C{s})/4                
                Ct{s}{w}(ceil(i*sz{w}(1)/sz{1}(1)),ceil(j*sz{w}(2)/sz{1}(2)))=treash_logic(w)*C{s}{w}(ceil(i*sz{w}(1)/sz{1}(1)),ceil(j*sz{w}(2)/sz{1}(2)))*Ct{s}{w}(ceil(i*sz{w}(1)/sz{1}(1)),ceil(j*sz{w}(2)/sz{1}(2)));
            end                        
        end
    end
    ...
    for  i=1:sz{1+length(C{s})/4}(1)
        for j=1:sz{1+length(C{s})/4}(2)
            ...
            for w = 1+length(C{s})/4:2*length(C{s})/4    
                i_c=ceil(i*sz{w}(1)/sz{1+length(C{s})/4}(1));
                j_c=ceil(j*sz{w}(2)/sz{1+length(C{s})/4}(2));
                Ct{s}{w}(i_c,j_c)=treash_logic(w)*C{s}{w}(i_c,j_c)*Ct{s}{w}(i_c,j_c);
            end                        
        end
    end
