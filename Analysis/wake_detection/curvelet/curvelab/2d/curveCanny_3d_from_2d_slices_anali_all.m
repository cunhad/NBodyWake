...
% old
nowake=reshape(permute(anali_depth(1,1:length(sample_list_nowake),:,4),[1,3,2,4,5]),[1,numel(anali_depth(1,1:length(sample_list_nowake),:,2,1))])
wake=reshape(permute(anali_depth(2,1:length(sample_list_wake),:,4),[1,3,2,4,5]),[1,numel(anali_depth(1,1:length(sample_list_wake),:,2,1))])
mean_wake=mean(wake)
mean_nowake=mean(nowake)
std_nowake=std(nowake,1)
stn_nowake=(nowake-mean_nowake)/std_nowake
