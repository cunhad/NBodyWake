...
%                 thresh=0;
                ...
                Ct_aux=cell(length(Ct{s}),1);
                parfor w = 1:length(Ct_aux)
%                     Ct{s}{w} = C{s}{w};
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > 0*E{s}{w});
%                     Ct{s}{w} = C{s}{w}.* ((C{s}{w}) > thresh*E{s}{w});
                    Ct_aux{w} = C{s}{w}.* ((C{s}{w}) > -thresh*E{s}{w}&(C{s}{w}) < thresh*E{s}{w});
%                 Ct{s}{w} = C{s}{w};
                    curv(w_nw,sample,slice_id,w,aux_count)=kurtosis(abs(Ct_aux{w}(:)));
                end
                Ct{s}=Ct_aux;
