        ...
        for partition=0:partition3rd-1
                for partition_x=0:partition2d-1
                    for partition_y=0:partition2d-1
                        ...
                        map_3d_slices_depth=map_3d_slices((partition_x)*nc/partition2d+1:(partition_x)*nc/partition2d+nc/partition2d,(partition_y)*nc/partition2d+1:(partition_y)*nc/partition2d+nc/partition2d,(partition)*slices/partition3rd+1:(partition)*slices/partition3rd+slices/partition3rd);
...
%                         old result
                        ...
                        map_3d_slices_depth2(:,:)=sum(map_3d_slices_depth(:,:,:),3);
                        
                        theta = 0:step_of_degree:180;
                        [R,xp] = radon(map_3d_slices_depth2,theta);
...
%                         anali_depth(slice_depth_id,4,:)=[max(R_nor_filt(:)),std(R_nor_filt(:)),max(R_nor_filt(:))/std(R_nor_filt(:)),kurtosis(kurtosis(R_nor_filt)),kurtosis(R_nor_filt(:))];
                        ...
                        anali3s(w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d,:)=[max(R_nor_filt(:)),std(R_nor_filt(:)),max(R_nor_filt(:))/std(R_nor_filt(:)),kurtosis(kurtosis(R_nor_filt)),kurtosis(R_nor_filt(:))];
                        d3s(:)=(anali3s(w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d,:));
                        
                        test4max3_lvs{sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d}=   plot(ax3_test4s,d3s,coul);
...
% 
%                         
%                         [ Radon_Y_ones,interp_Y_info,sample_points_Y_id,ncx_,ncy_,ncz_] = Radon3d_interp_forwards_dev3(ones(size(permute(map_3d_slices_depth,[1 3 2]))));
%                         [ Radon_Y_,interp_Y_info,ncx_,ncy_,ncz_] = Radon3d_interp_forwards_dev3(permute(map_3d_slices_depth,[1 3 2]));
%                         Radon_Y_norm_ = real(Radon_Y_)./real(Radon_Y_ones);
%                         Ridgelet_Y = Ridgelet3d_fromRadon_dev3(Radon_Y_norm_,sample_points_Y_id,lev_3drig);
% %                         dataY = Radon_Y_norm_(1,:);
%                         Ridgelet_Y(isnan(Ridgelet_Y(:))) = [];
%                         dataY = Ridgelet_Y;
%                         
%                         [ Radon_X_ones,interp_X_info,sample_points_X_id,ncx_,ncy_,ncz_] = Radon3d_interp_forwards_dev3(ones(size(permute(map_3d_slices_depth,[3 2 1]))));
%                         [ Radon_X_,interp_X_info,ncx_,ncy_,ncz_] = Radon3d_interp_forwards_dev3(permute(map_3d_slices_depth,[3 2 1]));
%                         Radon_X_norm_ = real(Radon_X_)./real(Radon_X_ones);
%                         Ridgelet_X = Ridgelet3d_fromRadon_dev3(Radon_X_norm_,sample_points_X_id,lev_3drig);
...
%                         
%                         
%                         anali3s(w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d,:)=[max(data(:)),std(data(:)),max(data(:))/std(data(:)),kurtosis(data(:))];
%                         d3s(:)=(anali3s(w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d,:));
%                         
%                         test4max3_lvs{sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d}=   plot(ax3_test4s,d3s,coul);
%                         clearvars a3s b3s c3s d3s
...
%                         if (w_nw==1&&sample==sample_id_range_nw(1)&&partition==0&&partition_x==0&&partition_y==0)
%                             fig = figure;
%                             h1 = histogram(data);
% %                             set(gca, 'YScale', 'log')
%                             Hist{w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d} = h1.BinCounts;
%                             BinWidth1 = h1.BinWidth;
%                             BinEdgesHist{w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d} = h1.BinEdges;
%                             close(fig);
%                                                         
...
% %                             set(gca, 'YScale', 'log')
%                             Hist{w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d} = h.BinCounts;
%                             BinEdgesHist{w_nw,sample,1+partition_x+partition_y*partition2d+partition*partition2d*partition2d} = h.BinEdges;
%                             close(fig);
%                             
...
domain = dom_min:BinWidth1:dom_max;

[a b c] = size(BinEdgesHist)

for i = 1:b    
    for j = 1:c
    Hist_comDom_nw(j+c*(i-1),:) = interp1(BinEdgesHist{1,i,j}(1:end-1)',Hist{1,i,j}',domain','nearest','extrap');
    end
end

for i = 1:b    
    for j = 1:c
    Hist_comDom_w(j+c*(i-1),:) = interp1(BinEdgesHist{2,i,j}(1:end-1)',Hist{2,i,j}',domain','nearest','extrap');
    end
end
...
nowake = reshape(anali3s(1,sample_id_range_nw,:,3),[length(sample_id_range_nw),partition3rd])'
wake  = reshape(anali3s(2,sample_id_range_w,:,3),[length(sample_id_range_w),partition3rd])'
nowake_sorted=sort(nowake)
wake_sorted=sort(wake)
nowake_sig=sum(nowake_sorted)
wake_sig=sum(wake_sorted)
mean_nowake=mean(nowake_sig)
mean_wake=mean(wake_sig)
std_nowake=std(nowake_sig,1)
std_wake=std(wake_sig,1)
stn_nowake=(nowake_sig-mean_nowake)/std_nowake
stn_wake=(wake_sig-mean_nowake)/std_nowake
...
% 
% nowake=reshape(anali3s(1,sample_id_range_nw,:,3),[1,numel(anali3s(1,sample_id_range_nw,:,1))])
% wake=reshape(anali3s(2,sample_id_range_w,:,3),[1,numel(anali3s(1,sample_id_range_w,:,1))])
% mean_wake=mean(wake)
% mean_nowake=mean(nowake)
% std_nowake=std(nowake,1)
% stn_nowake=(nowake-mean_nowake)/std_nowake
...
% 
% 
% % wake_slices = reshape(wake,[partition3rd,length(sample_list_wake)])'
% % nowake_slices = reshape(nowake,[partition3rd,length(sample_list_nowake)])'
% wake_slices = reshape(wake,[partition3rd,length(sample_id_range_w)])'
% nowake_slices = reshape(nowake,[partition3rd,length(sample_id_range_nw)])'
% max_wake_slices_=sort(wake_slices')
% max_nowake_slices_=sort(nowake_slices')
