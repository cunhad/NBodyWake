function [ anali_sum2,sample_id_range_nw,sample_id_range_w ] = curve3d_show_slices_minimal_nor_test(  )
...
%             end
            ...
            for s = length(C_)-lev:length(C_)-1
                ...
                for w = 1:length(C_{s})
                    ...
                    %                 Ct{s}{w} = C{s}{w};
%                     curv(w_nw,sample,slice_id,w,aux_count)=kurtosis(abs(C{s}{w}(:)));
                    collect_=[ collect ;abs(C_{s}{w}(:))];
                    collect=[ collect_ ;abs(C_{s}{w}(:))/E{s}{w}];
                    
                    aver=mean(abs(C_{s}{w}(:)));
                    sigma=std(abs(C_{s}{w}(:)))^2;
                    delta=skewness(abs(C_{s}{w}(:)))*(sigma^(3/2));
                    rho=kurtosis(abs(C_{s}{w}(:)))*(sigma^(4));
                    
                    A=aver;
                    B=sigma+A^2;
                    C=delta+3*B*A-2*A^3;
                    D=rho+4*C*A+6*B*A^2+3*A^3;
                    
                    size_n=prod(size(C_{s}{w}(:)));                    
...
%                 curv(w_nw,sample,slice_id,w,aux_count)=kurtosis(abs(C{s}(:)));
                  ...
                  rho_t=DT-4*CT*AT+6*BT*AT^2+3*AT^3;
%                   aver_t=;
                ...
                curv_mom3(aux_count)=kurt_t;
                curv_mom4(aux_count)=skew_t;
...
%             test1_lv{sample_id}=   plot(ax_test1,a,coul);
            ...
            clearvars b c d a_curv b_curv a_curv_mom b_curv_mom c_curv_mom d_curv_mom
%             
%             %         clearvars R R_nor R_nor_filt kurt2;
%             
%             hold(ax_curv,'on');
...
title(ax_test1_curv_mom,'average normalised');
set(ax_test2_curv_mom, 'YScale', 'log');
title(ax_test2_curv_mom,'std curvelet');
set(ax_test3_curv_mom, 'YScale', 'log');
title(ax_test3_curv_mom,'skewness normalised');
set(ax_test4_curv_mom, 'YScale', 'log');
title(ax_test4_curv_mom,'kurtosis curvelet');
