function [ anali,sample_id_range_nw,sample_id_range_w ] = curve2d_show_slices_minimal_nor_sabs(  )
...
display_slice_nw{find(sample_id_range_nw==7)}=[23];
% display_slice_w{find(sample_id_range_w==3)}=[29];
display_slice_w{find(sample_id_range_w==7)}=[23];
...
%             BW2 = abs(ifdct_wrapping(Ct,0));
            ...
            BW2=sign(real(ifdct_wrapping(Ct,0))).*(abs(ifdct_wrapping(Ct,0)));
            
            map_3d_slices_filt2d(:,:,slice_id) =sign(real(ifdct_wrapping(Ct,0))).*(abs(ifdct_wrapping(Ct,0)));
...
% 
% [ anali,sample_id_range_nw,sample_id_range_w ] = curve2d_show_slices_minimal_nor_sabs(  )
% 
% 
