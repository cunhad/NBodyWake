function [ map,anali ] = slices_curv_2d_locangreal( root,root_data_2d_in,root_data_2d_anali,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,numb_rand,slices,lev,lev_rid,step_of_degree,wavel_removal_factor,NSIDE,partition2d,sum_depth)
...
anali_curv=zeros(slice,5,lev);
anali_depth=zeros(slice/sum_depth,4,5);
anali_curv_depth=zeros(slice/sum_depth,5,lev);

anali3_curv(5,lev_3d);

anali2a3=zeros(slice,4,5);
anali2a3_curv=zeros(slice,5,lev);
anali2a3_depth=zeros(slice/sum_depth,4,5);
anali2a3_curv_depth=zeros(slice/sum_depth,5,lev);
...
%     R_nor_filt(1:217+256,:)=[];
    ...
    anali(slice_id,4,:)=[max(R_nor_filt(:)),std(R_nor_filt(:)),max(R_nor_filt(:))/std(R_nor_filt(:)),kurtosis(kurtosis(R_nor_filt)),kurtosis(R_nor_filt(:))];
        ...
        for partition=0:1
            ...
            map_3d_slices_filt2a3d(:,:,(partition)*slice/2+1:(partition)*slice/2+slice/2) = real(fdct3d_inverse(Ct));
