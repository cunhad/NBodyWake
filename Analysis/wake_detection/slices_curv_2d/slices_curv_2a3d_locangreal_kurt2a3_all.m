function [ anali,signal,equator_phi ] = slices_curv_2a3d_locangreal_kurt2a3_all( root,root_anali_2d_in,root_2d_anali_hpx,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,slice,NSIDE,analy_type,analy_lev)
...
% root_2d_anali_hpx='/home/asus/Dropbox/extras/storage/graham/ht/data_cps32_1024_2dclarhpxNSIDE8l2lr1na256_to_3dparcurv-2dclarhpxNSIDE8l1lr1_analiKurt1_hpx/';
...
for angle_id=1:N_angles
    ...
    aux=dlmread(strcat(path_in,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_3dcurv_z',num2str(z_glob),'_anali2a3_curv','.txt'));
    
    
    number_lv=prod(size(aux))/(slice*5);
    anali(angle_id,:,:,:)=reshape(aux,slice,5,number_lv);
...
signal=anali(:,:,analy_type,analy_lev);
...
signal_det=anali(:,:,analy_type,analy_lev);
