function [ anali,signal,equator_phi ] = slices_curv_2d_all( root,root_anali_2d_in,root_2d_anali_hpx,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,slice,NSIDE)

%(example)  [ anali,signal,equator_phi ] = slices_curv_2d_all('/home/asus/Dropbox/extras/storage/graham/small_res/','/home/asus/Dropbox/extras/storage/graham/small_res/anali/','/home/asus/Dropbox/extras/storage/graham/small_res/anali_hpx/','64Mpc_256c_128p_zi63_nowakem','/sample2001/','','10.000xv0.dat',1,1,2,2 );
...
for angle_id=1:N_angles
    ...
    path1=strcat(root_anali_2d_in,spec,aux_path,'data/',aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf','/NSIDE_',num2str(NSIDE),'/anglid_',num2str(angle_id),'/')
    ...
    path_in=string(strcat(strcat(root_anali_2d_in,spec,aux_path),'data/',aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf','/NSIDE_',num2str(NSIDE),'/anglid_',num2str(angle_id),'/',path2,'/2dproj/dm/'))
    
    aux=dlmread(strcat(path_in,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_z',num2str(z_glob),'_data_curv_sl','.txt'));
...
tot_anali_path_out=char(strcat(root_2d_anali_hpx,spec,aux_path,'anali/',aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf','/NSIDE_',num2str(NSIDE),'/slices_curv_2d/hpx/'))
mkdir(tot_anali_path_out);
        
tot_anali_data_path_out=char(strcat(root_2d_anali_hpx,spec,aux_path,'data_anali/',aux_path_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf','/NSIDE_',num2str(NSIDE),'/slices_curv_2d/hpx/'))
...
% fig=figure
 ...
 [x, y] = ssht_mollweide(thetas, phis,0,0);
    ...
    saveas(fig,strcat(tot_anali_path_out,strcat('/molvp'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_molvp_hpx_slice_cuvr_2d_z',z,'_plot.png'));
   
    mkdir(tot_anali_data_path_out,strcat('/molvd/'));
    dlmwrite(strcat(tot_anali_data_path_out,strcat('/molvd'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_molvp_hpx_slice_cuvr_2d_z',z,'_data.txt'),[thetas,phis,signal'],'delimiter','\t')
    dlmwrite(strcat(tot_anali_data_path_out,strcat('/molvd'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_molvp_hpx_slice_cuvr_2d_z',z,'_data_det.txt'),signal_det,'delimiter','\t')
...
set(gca,'linewidth',2);
    ...
    saveas(fig,strcat(tot_anali_path_out,strcat('/equator_det'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_equ_d_slice_cuvr_2d_z',z,'_plot.png'));
   hold off
   
   mkdir(tot_anali_data_path_out,strcat('/equator_data/'));
    dlmwrite(strcat(tot_anali_data_path_out,strcat('/equator_data'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_equdata_slice_cuvr_2d_z',z,'_data.txt'),[equator_phi,equator_signal'],'delimiter','\t')
    dlmwrite(strcat(tot_anali_data_path_out,strcat('/equator_data'),'/_',num2str(find(str2num(char(redshift_list))==z_glob)),'_equdata_slice_cuvr_2d_z',z,'_data_summary.txt'),[equator_phi(max_indx),max(equator_signal(:)),std(equator_signal(:)),max(equator_signal(:))/std(equator_signal(:))],'delimiter','\t')
