...
for w_nw=1:2
    ...
    for sample = 1:length(sample_list)
        ...
        for angle_id=1:length(list_of_angle_paths)
            ...
            %depth analysis
% 
%             filename=strcat(specs_path_list,'/',string(sample_list(sample)),'/data_2d_filt_slices/1lf_1rf','/NSIDE_',num2str(NSIDE),'/',list_of_angle_paths(angle_id),'/',string(path2),'/2dproj/dm/',ch,'_2dproj_curv_z3_anali2a3_depth4.txt');                        
%             
%             info = dlmread(filename);
%             
%             anali(angle_id,:,:,:)=reshape(info,slices/4,4,5);
%             
%             signal=anali(:,:,4,analy_type);
%             
%             signal=sum(signal,2);
