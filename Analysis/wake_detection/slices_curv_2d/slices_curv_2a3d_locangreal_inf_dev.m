...
% root_visual_2d='/home/asus/Dropbox/extras/storage/graham/ht/data_tst_anali_visual/';
...
% % snapshot=[13,28]*(256/32);
% % snapshot=[9,29]*(128/32);
% visual_type=[1:3];        %if 1, shows the 2d proj; %if 2 shows the ridgelet transformation    %if 3 does everything above also for the depth                          
% visual_in_or_out=[1:3]; %if 1 do visualization of the input,%  if 2 of the 2d_filtered    %  if 3 of the 3d filtered                            
% %  visual_in_or_out=[2];
% stage=[3];              % for doing the 3d
% %  info=[0,1,2]             %0,for just simplest visualisation
...
%     map_3d_slices(:,:,slice_id)=log(2+map_dc);
...
%     Ct2=C;    
    ...
    dlmwrite(strcat(path_data_2d_anali_out,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_2dcurv_z',num2str(z_glob),'_anali2_depth',num2str(sum_depth),'.txt'),anali_depth,'delimiter','\t');
    dlmwrite(strcat(path_data_2d_anali_out,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_3dcurv_z',num2str(z_glob),'_anali2_curv_depth.txt'),anali_curv_depth,'delimiter','\t');
...
if ismember(3,stage)
    ...
    if ~ismember(1,sum_depth)
        ...
        dlmwrite(strcat(path_data_2d_anali_out,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_2dcurv_z',num2str(z_glob),'_anali_depth',num2str(sum_depth),'.txt'),anali2a3_depth,'delimiter','\t');
        dlmwrite(strcat(path_data_2d_anali_out,'_',num2str(find(str2num(char(redshift_list))==z_glob)),'_2dproj_3dcurv_z',num2str(z_glob),'_anali2a3_curv_depth.txt'),anali2a3_curv_depth,'delimiter','\t');
    end
end
% 
