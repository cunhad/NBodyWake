function [] = Produce_fig_from3dbin_eachSim_terminal(path_in,path_out,spec,aux_path,NSIDE)
...
% % aux_path = '/sample5001/half_lin_cutoff_half_tot_pert_nvpw_v0p6/';
% NSIDE = 4;
...
sample_part_out = matches{1}{1};  % Get the first match


sample_list_out_path=strcat(path_out,spec,'/',sample_part_out,'/');
mkdir(sample_list_out_path);

specs_path_list = strcat(path_in,spec,aux_path);

list_of_angle_paths=dir(char(strcat(specs_path_list,'/data','/1lf_0.5rf','/NSIDE_',num2str(NSIDE),'/anglid_*')));
...
for angle_id=1:length(list_of_angle_paths)
    %       for angle_id=1
    path_prefix_name_out = strcat(sample_list_out_path,sample_part_out,'-',list_of_angle_paths(angle_id));
    %             mkdir(prefix_name);

    path1=strcat(specs_path_list,'data','/1lf_0.5rf','/NSIDE_',num2str(NSIDE),'/',list_of_angle_paths(angle_id),'/');
    path2=dir(char(strcat(path1,"*pv*")));
    path2={path2.name};
    path2=path2(1);
    file_in = strcat(specs_path_list,'data','/1lf_0.5rf','/NSIDE_',num2str(NSIDE),'/',list_of_angle_paths(angle_id),'/',string(path2),'/2dproj/dm/',filename_in,'.bin');
    fid = fopen(file_in);

    for slice_id=1:slices
        %             for slice_id=1
        file_out = char(strcat(path_prefix_name_out,'-',filename_out,num2str(slice_id),'.png'));
...
%         % Remove the ticks from both the x-axis and y-axis
%         set(gca, 'XTick', [], 'YTick', []);
        saveas(fig,file_out);
        close(fig);
    end
