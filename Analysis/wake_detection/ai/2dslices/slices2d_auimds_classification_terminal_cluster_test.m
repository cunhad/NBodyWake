function [  ] = slices2d_auimds_classification_terminal_cluster_test()
...
%         map_3d_slices_filt2d=zeros(nc,nc,slices);
        ...
        list_of_angle_paths=dir(char(strcat(specs_path_list,'/',string(sample_list(sample)),'/data/1lf_1rf','/NSIDE_',num2str(NSIDE),'/anglid_*')));
...
%                 %             fclose(fid);
                ...
                path1=strcat(specs_path_list,'/',string(sample_list(sample)),'/data/1lf_1rf','/NSIDE_',num2str(NSIDE),'/',list_of_angle_paths(angle_id),'/');
