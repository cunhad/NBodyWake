function [] = Produce_fig_from3dbin_terminal(sample_id)


% without serialyzing the samples, a memory problem arises
...
path_in = '/home/cunhad/projects/rrg-rhb/cunhad/simulations/cubep3m/ht/data_cps32_512_hpx_2d_NSIDE4' 
path_out = '/scratch/cunhad/soft_links/data_cps32_512_hpx_2d_NSIDE4'
...
sample_list_nowake_out=strcat(path_out,'/',specs_path_list_in(3).name);
sample_list_wake_out=strcat(path_out,'/',specs_path_list_in(4).name);
...
% for w_nw=1   
    if w_nw==1
        ...
        sample_list_out = sample_list_nowake_out;
    end    
    if w_nw==2
...
%       for angle_id=1
            path_out_complete = strcat(sample_list_out,'/',string(sample_list(sample)),'/',list_of_angle_paths(angle_id));
            mkdir(path_out_complete);
            ...
            path2={path2.name};
            path2=path2(1);
            file_in = strcat(specs_path_list,'/',string(sample_list(sample)),'/data','/1lf_0.5rf','/NSIDE_',num2str(NSIDE),'/',list_of_angle_paths(angle_id),'/',string(path2),'/2dproj/dm/',filename,'.bin');
            fid = fopen(file_in,'r');

            for slice_id=1:slices
%             for slice_id=1
                file_out = strcat(path_out_complete,'/',filename,num2str(slice_id),'.png');
