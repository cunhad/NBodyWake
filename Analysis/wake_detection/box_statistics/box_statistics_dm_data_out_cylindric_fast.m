function [ proj1d_angles ] = box_statistics_dm_data_out_cylindric_fast_to_gpu_vol( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,data_stream,level_window,dwbasis)
...
%(example)  [proj1d_angles] = box_statistics_dm_data_out_cylindric_fast_to_gpu_vol('/home/asus/Dropbox/extras/storage/graham/small_res/', '/home/asus/Dropbox/extras/storage/graham/small_res/box_stat_cylindric_fast_to_gpu_vol/','64Mpc_96c_48p_zi255_wakeGmu5t10m6zi63m','/sample1001/','','10.000xv0.dat',2,1,[0,0,0],2,1,4,[1,2],[1],'db1');
...
for part_id = 1  :   part
    ...
    [ ~, ~, ~, ~, ~ ,~ ,~ ,~ ,z, ~, Pos  ] = preprocessing_part(root,spec,aux_path,filename,part,part_id);
        ...
        liminf_pre=-(1/(2*lenght_factor_pre))*nc;
        limsup_pre= (1/(2*lenght_factor_pre))*nc;
...
for angl=1:number_of_angle_nuple_hpx/2
 ...
 phi_indice_inverse=theta_inverse_indices(find(angles_hpx(2,angl)==angles_hpx(2,theta_inverse_indices)));
 
    
    proj1d_angles(:,phi_indice_inverse)=proj1d_angles(:,angl);
