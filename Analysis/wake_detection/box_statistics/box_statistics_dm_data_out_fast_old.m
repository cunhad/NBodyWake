...
%     angl_indx(1)=1;
%     angl_indx(num_cores+1)=number_of_angle_nuple_hpx+1;
%     n_angl_indx=[];
    for cr=1:num_cores
...
% 
%     for cr=1:num_cores
%         angles_parfor1(cr)= [angles_hpx(1,angl_indx(cr):(angl_indx(cr+1)-1))];
%         angles_parfor2(cr)= [angles_hpx(2,angl_indx(cr):(angl_indx(cr+1)-1))];
%     end
...
%for k = 3:-1:1
    ...
    histogr_sum_parts=zeros(num_cores,n_angle_per_node,length(bins)-1);
...
for part_id = 1  :   part
    ...
    histogr(:,:,:)=zeros(num_cores,n_angle_per_node,length(bins)-1);
%     
%      for i=1:num_cores
     parfor i=1:num_cores
        ...
        angl_ind_start=angl_indx(i);
        angl_ind_end=angl_indx(i+1)-1;
...
%         angles2(:)=angles_parfor2(i,:);
% %         
[~,number_of_angle_nuple] = size(angles1);
...
%         nz=nz*Rz;
%         nz=nz*Ry;
        nx=[transpose(cos(angles1).*cos(angles1)) ,transpose(cos(angles1).*sin(angles2)), transpose(-sin(angles1))];
        ny=[transpose(-sin(angles2)) ,transpose(cos(angles2)), zeros(number_of_angle_nuple,1)];
%         nz=[sin(theta)*cos(phi) sin(theta)*sin(phi) cos(theta)];
        nz=[transpose(sin(angles1).*cos(angles2)) ,transpose(sin(angles1).*sin(angles2)), transpose(cos(angles1))];
        %
%         rx=Rz*rx;
%         rx=Ry*rx;
...
%         rx(:,dx<=liminf|dx>=limsup|dy<=liminf|dy>=limsup|dz<=liminf|dz>=limsup)=[];
        ...
        dz((nx*rx).*(nx*rx)+(ny*rx).*(ny*rx)>=limsup_sq|dz<=liminf|dz>=limsup)=NaN;
        
        xc = mat2cell(dz, ones(1,size(dz,1)), size(dz,2));
        [hcell,hedges] = cellfun(@(dz) histcounts(dz,bins), xc, 'Uni',0);
        histogr(i,:,:) = cell2mat(hcell);
        
%         for angl=1:number_of_angle_nuple
%         histogr = histcounts(dz(angl,:),bins);
%         proj1d_angles(:,angl)=proj1d_angles(:,angl)+transpose(histogr(1,:));%         rx=transpose(rx);
%         end

%         for angl=1:number_of_angle_nuple
%             for j=1:(lenght(bins)-1)
%             a(j)=proj1d_angles(j,angl);
%             end
%         proj1d_angles(:,angl)=a;
%         %         rx=transpose(rx);
%         end
...
%             
        ...
        histogr_sum_parts=histogr_sum_parts+histogr;
