function [ pks,locs,out_proj1d_angles ,out_filtered_proj1d_angles,f_index_max,f_max,thetas_max,phis_max] = box_statistics_dm_analysis_to_curvelet( root,root_data_out,root_plot_out,spec,aux_path,aux_path_data_out,aux_path_plot_out,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,lim,data_stream,info,analysis ,cutoff,projection_analysed,signal_analysed)

%(example)  [  ] = box_statistics_dm_analysis_to_curvelet('/home/asus/Dropbox/extras/storage/graham/small_res/', '/home/asus/Dropbox/extras/storage/graham/small_res/box_stat/','/home/asus/Dropbox/extras/storage/graham/small_res/box_stat/','64Mpc_96c_48p_zi255_wakeGmu5t10m6zi63m','/sample1001/','','','10.000xv0.dat',2,2,[0,0,0],2,1,4,'minmax',[1,3],[0,1,2,3],1,10,[1,2,3,4],[1,2,3]);

%(example)  [  ] = box_statistics_dm_analysis_to_curvelet('/home/asus/Dropbox/extras/storage/guillimin/', '/home/asus/Dropbox/extras/storage/guillimin/box_stat/','/home/asus/Dropbox/extras/storage/guillimin/box_stat/','64Mpc_1024c_512p_zi63_wakeGmu1t10m7zi31m','/sample0001/','','','10.000xv0.dat',2,2,[0,0,0],64,16,4,'minmax',[1],[0,1,2,3],1,0.8,[3],[4]);
...
figure;
plot(filtered_proj1d_angles(:,f_index_max(1:n_angle_peaks)));

peaks_tot=[];
for pk_angl_indx=1:n_angle_peaks
    [pks,locs] =findpeaks(filtered_proj1d_angles(:,f_index_max(pk_angl_indx)),'NPeaks',n_1d_peaks,'SortStr','descend');
...
path_data_out_curv=strcat(strcat(root_data_out(1,1:end-1),'_curv/',spec,aux_path),'peaks_info/',aux_path_data_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','stat/box_statistics/dm/');
mkdir(strcat(strcat(root_data_out(1,1:end-1),'_curv/',spec,aux_path),'peaks_info/',aux_path_data_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','stat/box_statistics/dm/'));
...
% [f_index_max,f_max,thetas_max,phis_max] = s2let_hpx_plot_mollweide_findpeaks(f_wav{J-J_min+1}+f_wav{J-J_min}+f_wav{J-J_min-1},1); 
