...
%     quantity_type=1;
    ...
    if dm_or_dc==1
        ...
        tot_plot_path_out=strcat(root_plot_out,spec,aux_path,'plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
        mkdir(tot_plot_path_out);
        
        snan_tot_path_out=strcat(root_snan_out,spec,aux_path,'snan/',aux_path_snan_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
    ...
    if dm_or_dc==2
        ...
        tot_plot_path_out=strcat(root_plot_out,spec,aux_path,'plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/dc/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
        mkdir(tot_plot_path_out);
        
        snan_tot_path_out=strcat(root_snan_out,spec,aux_path,'snan/',aux_path_snan_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/dc/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
        mkdir(snan_tot_path_out);
        
    end

% tot_plot_path_out=strcat(root_plot_out,spec,aux_path,'plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
% mkdir(tot_plot_path_out);
% 
% snan_tot_path_out=strcat(root_snan_out,spec,aux_path,'snan/',aux_path_snan_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/');
% mkdir(snan_tot_path_out);
...
fileID = fopen(strcat(path_data,'level_window',mat2str(level_window(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_filtered_dc_1dproj_angle_z',num2str(z),'_anglparts',num2str(angle_p),'_NSIDE',num2str(NSIDE),'_full.bin'));
% display(strcat(path_data,'level_window',mat2str(level_window(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part_in),'_NSIDE',num2str(NSIDE),'.bin'))
display(strcat(path_data,'level_window',mat2str(level_window(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_filtered_dc_1dproj_angle_z',num2str(z),'_anglparts',num2str(angle_p),'_NSIDE',num2str(NSIDE),'_full.bin'));
out_filtered_proj1d_angles=fread(fileID,[3,12*NSIDE^2],'float32','l');
fclose(fileID);
end
...
% now we define the peak square over standart deviation, 

% psqtstd=zeros(1,12*NSIDE^2);



if (quantity_type==4)   %   psqtstd

psqtstd(1,:)=(out_filtered_proj1d_angles(1,:).^2)./(out_filtered_proj1d_angles(3,:));

% out_filtered_proj1d_angles(1:3,:)=psqtstd(1,:);
out_filtered_proj1d_angles(1,:)=psqtstd(1,:);
out_filtered_proj1d_angles(2,:)=psqtstd(1,:);
out_filtered_proj1d_angles(3,:)=psqtstd(1,:);
quantity_type=1;
...
psqtstd(1,:)=out_filtered_proj1d_angles(1,:).*(out_filtered_proj1d_angles(2,:));
...
psqtstd(1,:)=(out_filtered_proj1d_angles(1,:).^2)./(out_filtered_proj1d_angles(2,:));
...
peaks_maxima_info=zeros(3,n_max_);
...
%     fig2=figure;
    ...
    camup([0 1 0])
    zoom(n_angles_1d);
     saveas(fig2,strcat(tot_plot_path_out,strcat('/peak/','max_',num2str(max_id)),'/zoom/nangl_',num2str(n_angles_1d),'/_',num2str(find(str2num(char(redshift_list))==z)),'_box_moll_zoom_z',num2str(z),'_plot.png'));
     close(fig2)
...
%     col=colormap(parula(max_id))
    for max_id=1:n_max_
    plots{max_id}=scatter(x_max(max_id), y_max(max_id),255,'r'); 
    leg{max_id}=strcat('peak= ',num2str(peaks_maxima_info(1,max_id)));
%     scatter(x_max, y_max,255);
 descr = {strcat('max = ',num2str(peaks_maxima_info(1,max_id)));
        strcat('std = ',num2str(std(out_filtered_proj1d_angles(quantity_type,:))));
        strcat('stn = ',num2str(peaks_maxima_info(1,max_id)/std(out_filtered_proj1d_angles(quantity_type,:))))};    
    ax1 = axes('Position',[0 0 1 1],'Visible','off');
    txt=text(0.75,0.5,descr);
    set(txt,'Parent',ax1,'interpreter', 'latex');
    ...
    %here we will construct the localized spherical map with the filtered
    %1d projection
...
% cd ../wake_detection/box_statistics/
    ...
    fileID = fopen(strcat(path_data_box_all,'level_window',mat2str(level_window(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_1dproj_angle_z',num2str(z),'_anglparts',num2str(angle_p),'_NSIDE',num2str(NSIDE),'_full.bin'));
    fseek(fileID,(length(bins)-1)*(f_index_max(max_id)-1)*4,'bof');
            proj_1d_peak=fread(fileID,[length(bins)-1],'float32','l');
            fclose(fileID);
            theta_max=peaks_maxima_info(2,1);
            phi_max=peaks_maxima_info(3,1);
            
            bin_of_peak=find(proj_1d_peak==max(proj_1d_peak));
            half_bin_number=(length(bins)-1)/2;
%             nz=[sin(theta)*cos(phi) sin(theta)*sin(phi) cos(theta)]
            
            center_peak(1,1)=(bin_of_peak-half_bin_number)*sin(theta_max)*cos(phi_max);
            center_peak(2,1)=(bin_of_peak-half_bin_number)*sin(theta_max)*sin(phi_max);
            center_peak(3,1)=(bin_of_peak-half_bin_number)*cos(theta_max);
            
    fileID = fopen(strcat(path_data_box_all,'level_window',mat2str(level_window(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_1dproj_angle_z',num2str(z),'_anglparts',num2str(angle_p),'_NSIDE',num2str(NSIDE),'_full.bin'));
            
    for pix=1:12*NSIDE*NSIDE
        theta=thetas(pix);
        phi=phis(pix);
        nz=[sin(theta)*cos(phi) sin(theta)*sin(phi) cos(theta)];
        
        bin_local=(nz*center_peak)+half_bin_number;
        
        fseek(fileID,((length(bins)-1)*(pix-1)*4)+(floor((bin_local-1))*4),'bof');
%         fseek(fileID,(length(bins)-1)*(pix-1)*4,'bof');
        spherical_local(pix,1)=fread(fileID,1,'float32','l');
...
%         fig3=figure;
        ...
        ax1 = axes('Position',[0.05 0.13 0.7 0.7]);
        gridDelaunay = delaunay(x,y);
        h = trisurf(gridDelaunay,x,y,spherical_local(:)*0.0,spherical_local(:));
        colorbar('southoutside');
        set(h, 'LineStyle', 'none')
        hold on;
        axis equal
        ...
        campos([0 0 1])
        camup([0 1 0])
        [x_max, y_max] = ssht_mollweide(theta_max, phi_max,0,0);
        scatter(x_max, y_max,255,'r');
        descr = {strcat('max = ',num2str(max(spherical_local(:))));
            strcat('std = ',num2str(std(spherical_local(:))));
            strcat('stn = ',num2str(max(spherical_local(:))/std(spherical_local(:))))};
        ax1 = axes('Position',[0 0 1 1],'Visible','off');
        txt=text(0.75,0.5,descr);
        set(txt,'Parent',ax1,'interpreter', 'latex');
        hold off;
...
[f_wav_tot, f_scal] = s2let_transform_axisym_analysis_hpx(spherical_local,'B',B,'L',L,'J_min',J_min);
...
for J_level_cutoff=1:J+1
  ...
  if dm_or_dc==1
        ...
        tot_plot_path_out=strcat(root_plot_out,spec,aux_path,'plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/s2let/J_lev_cut',num2str(J_level_cutoff),'/');
        mkdir(tot_plot_path_out);
        
        snan_tot_path_out=strcat(root_snan_out,spec,aux_path,'snan/',aux_path_snan_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/s2let/J_lev_cut',num2str(J_level_cutoff),'/');
        mkdir(snan_tot_path_out);
    ...
    if dm_or_dc==2
        ...
        tot_plot_path_out=strcat(root_plot_out,spec,aux_path,'plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/dc/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/s2let/J_lev_cut',num2str(J_level_cutoff),'/');
        mkdir(tot_plot_path_out);
        
        snan_tot_path_out=strcat(root_snan_out,spec,aux_path,'snan/',aux_path_snan_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','box/dm/dc/',dwbasis,'/','level_window',mat2str(level_window(:)),'/',type,'/parts/s2let/J_lev_cut',num2str(J_level_cutoff),'/');
        mkdir(snan_tot_path_out);
...
%     end
    ...
    pk(J_level_cutoff,1)=out_filtered_proj1d_angles_sort(1);
    mn_(J_level_cutoff,1)=mn;
    standard_dev(J_level_cutoff,1)=[std(f_out(:))];
    pk_o_mn(J_level_cutoff,1)=pk(J_level_cutoff,1)/mn;
    pk_o_std(J_level_cutoff,1)=pk(J_level_cutoff,1)/standard_dev(J_level_cutoff,1);
    pksq_o_std_t_mn(J_level_cutoff,1)=(pk(J_level_cutoff,1)^2)/(mn*standard_dev(J_level_cutoff,1));
    
    Table_info=table(lvl,pk,mn_,standard_dev,pk_o_mn,pk_o_std,pksq_o_std_t_mn);
