function [ proj1d_angles ] = box_statistics_dm_data_out_cubic_fast_spmd( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,pivot,NSIDE,part_p,num_cores,data_stream,level_window,dwbasis)
...
%(example)  [proj1d_angles] = box_statistics_dm_data_out_cubic_fast_spmd('/home/asus/Dropbox/extras/storage/graham/small_res/', '/home/asus/Dropbox/extras/storage/graham/small_res/box_stat_cubic_fast/','64Mpc_96c_48p_zi255_wakeGmu5t10m6zi63m','/sample1001/','','10.000xv0.dat',2,1,[0,0,0],2,1,4,4,[1,2],[1],'sym6');
...
angles_hpx(:,number_of_angle_nuple_hpx+1:end)=[];

angles_t=distributed(transpose(angles_hpx(1,:)));
angles_p=distributed(transpose(angles_hpx(2,:)));
...
histogr_1d_angles=zeros(number_of_angle_nuple_hpx,length(bins)-1);
histogr_1d_angles1=zeros(number_of_angle_nuple_hpx,length(bins)-1);
...
%     spmd 
        ...
        for i=drange(1:number_of_angle_nuple_hpx)
        ...
        histogr_1d_angles1(i,:)=histcounts(gather(dz),bins);
...
%     end
    ...
    proj1d_angles=proj1d_angles+transpose(gather(histogr_1d_angles1));
