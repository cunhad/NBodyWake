function [ out_filtered_proj1d_angles ] = box_statistics_dm_analysis_dwt_snan( root,root_data_in,root_plot_out,root_snan_out,spec,aux_path,aux_path_data_in,aux_path_plot_out,aux_path_snan_out,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,lim,data_stream,info,analysis,level_window_1dwavfilter,dwbasis,projection_analysed,signal_analysed,level_window_rm)

%(example)  [  ] = box_statistics_dm_analysis_dwt_snan('/home/asus/Dropbox/extras/storage/graham/small_res/', '/home/asus/Dropbox/extras/storage/graham/small_res/data_test/','/home/asus/Dropbox/extras/storage/graham/small_res/test_plot_box/','/home/asus/Dropbox/extras/storage/graham/small_res/test_snan_box/','64Mpc_96c_48p_zi255_wakeGmu5t10m6zi63m','/sample1001/','','','','0.000xv0.dat',2,1,[0,0,0],4,1,4,'minmax',[1,3],[0,1,2,3],1,[1],'db1',[1,2,3,4],[1,2,3],[0:1]);

%(example)  [  ] = box_statistics_dm_analysis_dwt_snan('/home/asus/Dropbox/extras/storage/guillimin/', '/home/asus/Dropbox/extras/storage/guillimin/box_stat_cubic_fast/','/home/asus/Dropbox/extras/storage/guillimin/box_stat_cubic_fast_plot/','/home/asus/Dropbox/extras/storage/guillimin/box_stat_cubic_fast_snan/','64Mpc_1024c_512p_zi63_wakeGmu1t10m7zi31m','/sample0001/','','','','10.000xv0.dat',2,1,[0,0,0],256,16,4,'minmax',[1],[0,1,2,3],1,[1],'sym6',[1,2,3,4],[1,2,3],[0:1]);
...
if ismember(0,data_stream)
    if (ismember(3,projection_analysed)||ismember(4,projection_analysed)||ismember(5,projection_analysed))
        [~,~,out_proj1d_angles,out_dc_proj1d_angles,out_filtered_proj1d_angles,out_filtered_dc_proj1d_angles,out_dc_filtered_proj1d_angles] =box_statistics_dm_data_out( root,root_data_in,spec,aux_path,aux_path_data_in,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,0,cutoff);
    else
        [~,~,out_proj1d_angles,out_dc_proj1d_angles,~,~,~] =box_statistics_dm_data_out( root,root_data_in,spec,aux_path,aux_path_data_in,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,0,cutoff);
    end
else
    path_data=strcat(strcat(root_data_in,spec,aux_path),'data/',aux_path_data_in,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','stat/box_statistics/dm/',dwbasis,'/');
    
    if ismember(2,data_stream)
        if ismember(3,data_stream)
            box_statistics_dm_data_out( root,root_data_in,spec,aux_path,aux_path_data_in,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,2,cutoff);
        end
    ...
    if ismember(1,data_stream)
        if ismember(3,data_stream)
            box_statistics_dm_data_out( root,root_data_in,spec,aux_path,aux_path_data_in,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,1,cutoff);
        end
...
%             out_dc_filtered_proj1d_angles=dlmread(strcat(path_data,'dc/','cutoff_',num2str(cutoff),'MpcCut/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_dc_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.txt'));
        ...
        if ismember(1,data_stream) 
            fileID = fopen(strcat(path_data,'level_window',mat2str(level_window_1dwavfilter(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.bin'));
...
%             fclose(fileID);
            ...
            fileID = fopen(strcat(path_data,'dc/','level_window',mat2str(level_window_1dwavfilter(:)),'/','_',num2str(find(str2num(char(redshift_list))==z)),'_out_filtered_dc_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.bin'));
...
mkdir(strcat(root_plot_out,spec,aux_path_plot_out),strcat('/plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv/','_NSIDE',num2str(NSIDE),'/','lw1d',mat2str(level_window_1dwavfilter(:)),'/lw_sp_rm',char(strjoin(string(level_window_rm),'-')),'/','dwb_',dwbasis,'/box/'));
tot_path_plot_out=strcat(root_plot_out,spec,aux_path_plot_out,'/plot/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','_NSIDE',num2str(NSIDE),'/','lw1d',mat2str(level_window_1dwavfilter(:)),'/lw_sp_rm',char(strjoin(string(level_window_rm),'-')),'/','dwb_',dwbasis,'/box/');

mkdir(strcat(root_snan_out,spec,aux_path_snan_out),strcat('/snan/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','_NSIDE',num2str(NSIDE),'/','lw1d',mat2str(level_window_1dwavfilter(:)),'/lw_sp_rm',char(strjoin(string(level_window_rm),'-')),'/','dwb_',dwbasis,'/box/'));
tot_path_snan_out=strcat(root_snan_out,spec,aux_path_snan_out,'/snan/',aux_path_plot_out,num2str(lenght_factor),'lf_',num2str(resol_factor),'rf_',strcat(num2str(pivot(1)),'-',num2str(pivot(2)),'-',num2str(pivot(3))),'pv','/','_NSIDE',num2str(NSIDE),'/','lw1d',mat2str(level_window_1dwavfilter(:)),'/lw_sp_rm',char(strjoin(string(level_window_rm),'-')),'/','dwb_',dwbasis,'/box/');
...
%         plot_molweide(transpose(out_proj1d_angles(1,:)),'Peak of the mass amplitude');
        mkdir(tot_path_plot_out,'peak_mass_amplit/');
        mkdir(tot_path_snan_out,'peak_mass_amplit/');
        save_plot=strcat(tot_path_plot_out,'peak_mass_amplit/','z',filename(1,end-11),'_box_pma.png');
        save_snan=strcat(tot_path_snan_out,'peak_mass_amplit/','z',filename(1,end-11),'_box_pma.txt');
        plot_molweide(transpose(out_proj1d_angles(1,:)),level_window_rm,save_plot,save_snan);
    end
...
function plot_molweide(f,level_window_rm,save_plot,save_snan)
...
nsideguessed = sqrt(max(sz)/12);
    ...
    for j = level_window_rm
    f_wav{j+1,1}(1,:)=0;
    end

    [f_s] = s2let_transform_axisym_synthesis_hpx(f_wav, f_scal,'B',B,'L',L,'J_min',J_min);
    
    s2let_hpx_plot_mollweide_info_save(f_s,save_plot,save_snan);
