function [ proj1d_angles] = box_statistics_dm_data_out_fast( root,root_out,spec,aux_path,aux_path_out,filename,lenght_factor,resol_factor,pivot,NSIDE,part,num_cores,data_stream,cutoff)
...
%(example)  [proj1d_angles] = box_statistics_dm_data_out_fast('/home/asus/Dropbox/extras/storage/graham/small_res/', '/home/asus/Dropbox/extras/storage/graham/small_res/box_stat/','64Mpc_96c_48p_zi255_wakeGmu5t10m6zi63m','/sample1001/','','10.000xv0.dat',2,2,[0,0,0],2,1,4,[1,2],4);
...
%         rx=Ry*(Rz*rx);
        ...
        rx=Rz*rx;
        rx=Ry*rx;   %25.732576 seconds. en nside16,4cores,nc96
%         
%         rx=full(sparse(Rz)*sparse(rx));
%         rx=full(sparse(Ry)*sparse(rx)); %41.732576 seconds. en nside16,4cores,nc96
% 
% rx=single(rx);
% Rz=single(Rz);
% Ry=single(Ry);
...
%         RX=RY*RX;
%         
%        rx = gather(RX); %14.183525 seconds seconds nside 8 node1 
...
%         mkdir(path_out_all,strcat('/dc/cutoff_',num2str(cutoff),'MpcCut/'));
%         
%     end
%     
...
%         if cutoff~=0 
%             
%             fileID = fopen(strcat(path_out_all,'cutoff_',num2str(cutoff),'MpcCut/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.bin'),'w');
%             fwrite(fileID,filtered_proj1d_angles, 'float32','l');
%             fclose(fileID);
%             
%             fileID = fopen(strcat(path_out_all,'dc/cutoff_',num2str(cutoff),'MpcCut/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_dc_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.bin'),'w');
...
%         
%         if cutoff~=0 
%             dlmwrite(strcat(path_out_all,'cutoff_',num2str(cutoff),'MpcCut/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.txt'),filtered_proj1d_angles,'delimiter','\t');
%             dlmwrite(strcat(path_out_all,'dc/cutoff_',num2str(cutoff),'MpcCut/','_',num2str(find(str2num(char(redshift_list))==z)),'_filtered_1dproj_angle_z',num2str(z),'_parts',num2str(part),'_NSIDE',num2str(NSIDE),'.txt'),filtered_dc_proj1d_angles,'delimiter','\t');
