function [ Radon_hor__,Radon_vert__] = Ridgelet2d_RP_crude_forwards_dev2(X)
...
for i=1:ncx+(1-is_odd_ncx)
    for j=1:ncy
        Radon_hor__(i,j)=(exp(-1i*pi*(j-1)*(1-is_odd_ncy/ncy)))*Radon_hor(i,j);
    end
end

for i=1:ncx
    for j=1:ncy+(1-is_odd_ncy)
        Radon_vert__(j,i)=(exp(-1i*pi*(i-1)*(1-is_odd_ncx/ncx)))*Radon_vert(j,i);
    end
end
